...
const refArbitroC = ref(dbEscrita, 'arbitros/arbitroC');

// Recuperar os dados do Firebase e preencher o campo de texto
onValue(refArbitroC, (snapshot) => {
    const arbitroC = snapshot.val();  // Recupera o valor de arbitroE no Firebase
    if (arbitroC) {
...
function carregarAtletas() {
  onValue(ref(dbLeitura, `avaliacaodejuradoF`), (snapshot) => {
    ...
    for (const fase in data) {
      for (const ritmo in data[fase]) {
        const juradoData = data[fase][ritmo]?.juradoF || {};
...
async function enviarNota() {
  ...
  const publico = parseFloat(document.getElementById("nota").value); // Nota do VJogoA
  const bonus = parseFloat(document.getElementById("floreio").value); // Nota do Solo

  // Verificação se as notas são válidas
  if (!dados || !faseSelecionada || isNaN(publico) || isNaN(bonus)) {
    alert("❌ Selecione um atleta e insira notas válidas.");
    return;
  }

  const confirmar = confirm(
    `Deseja realmente enviar as notas separadas (Publico: ${publico.toFixed(1)}, Bonus: ${bonus.toFixed(1)}) para ${dados.nome}?`
  );
  if (!confirmar) return;
  ...
  const dadosNota = {
    ...
    numero: dados.numero || "",
    jurado: JURADO,
    publico: publico.toFixed(1), // Envia a nota de VJogoA separada
    bonus: bonus.toFixed(1), // Envia a nota de Solo separada
    ritmo: dados.ritmo,
  ...
  try {
    ...
    alert(`✅ Notas enviadas separadas para ${dados.nome} (Publico: ${publico.toFixed(1)}, Bonus: ${bonus.toFixed(1)})`);
  } catch (error) {
