...
function carregarAtletas() {
  onValue(ref(dbLeitura, `avaliacaodejuradoD`), (snapshot) => {
    ...
    for (const fase in data) {
      for (const ritmo in data[fase]) {
        const juradoData = data[fase][ritmo]?.juradoD || {};
...
async function enviarNota() {
  ...
  const vjogoB = parseFloat(document.getElementById("nota").value); // Nota do VJogoA
  const aereo = parseFloat(document.getElementById("floreio").value); // Nota do Solo

  // Verificação se as notas são válidas
  if (!dados || !faseSelecionada || isNaN(vjogoB) || isNaN(aereo)) {
    alert("❌ Selecione um atleta e insira notas válidas.");
    return;
  }

  const confirmar = confirm(
    `Deseja realmente enviar as notas separadas (VJogoB: ${vjogoB.toFixed(1)}, Aereo: ${aereo.toFixed(1)}) para ${dados.nome}?`
  );
  if (!confirmar) return;
  ...
  const dadosNota = {
    ...
    numero: dados.numero || "",
    jurado: JURADO,
    vjogoB: vjogoB.toFixed(1), // Envia a nota de VJogoA separada
    aereo: aereo.toFixed(1), // Envia a nota de Solo separada
    ritmo: dados.ritmo,
  ...
  try {
    ...
    alert(`✅ Notas enviadas separadas para ${dados.nome} (VJogoB: ${vjogoB.toFixed(1)}, Aereo: ${aereo.toFixed(1)})`);
  } catch (error) {
